...
class DocumentModel extends Model
{
    ...
    public function update()
    {
        ...
        // 行为记录
        if ($id) {
            action_log('add_document', 'document', $id, is_login());
        }
     ...
     * @return mixed
     */
    public function myDocument($page = 1)
    {
        $list = $this
            ->where(array('uid' => is_login()))
            ->field('id,category_id,pid,title,description,view,create_time')
            ->order('id desc')
            ->page($page, 10)
            ->select();
        $data['list'] = $list;
        $data['total_page'] = $this->myDocumentPageCount();
     ...
     * @return mixed
     */
    public function myDocumentPageCount($map = array())
    {
        return $this->where($map)->where(array('uid' => is_login()))->count();
    }
}
